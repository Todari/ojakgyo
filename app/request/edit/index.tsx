...
export default function RequestEditPage() {
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          Alert.alert('알림', '등록된 도움 요청이 없습니다.', [
        ...
        if (error.code === '42P01') {
          Alert.alert('오류', 'help_requests 테이블이 존재하지 않습니다.', [
            { text: '확인', onPress: () => router.back() },
          ]);
          return;
        }
        ...
        Alert.alert('오류', '요청서를 불러오는데 실패했습니다.', [
          { text: '확인', onPress: () => router.back() },
        ]);
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (selectedCategories.length === 0) {
      Alert.alert('필수 입력', '도움 종류를 하나 이상 선택해주세요.');
      return;
    }
    if (!details.trim()) {
      Alert.alert('필수 입력', '추가 요청사항을 입력해주세요.');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        Alert.alert('오류', '요청서 수정 중 오류가 발생했습니다.');
        return;
      }

      Alert.alert('수정 완료', '도움 요청이 성공적으로 수정되었습니다.', [
        { text: '확인', onPress: () => router.back() },
      ]);
    } catch (e) {
      console.error('Unexpected error:', e);
          ...
          onPress: async () => {
            try {
              ...
              if (error) {
                ...
                Alert.alert('오류', '요청 삭제 중 오류가 발생했습니다.');
                return;
              }

              Alert.alert('삭제 완료', '도움 요청이 삭제되었습니다.', [
                { text: '확인', onPress: () => router.back() },
              ]);
            } catch (e) {
              console.error('Unexpected error:', e);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Typography variant='body' style={styles.subtitle}>
          등록된 도움 요청을 수정하거나 공개 상태를 변경할 수 있습니다.
        </Typography>
        ...
        <View style={styles.section}>
          ...
          <TextArea
            ...
            placeholder='필요한 도움에 대해 자세히 작성해주세요.'
            style={styles.input}
          />
        </View>
