...
export default function RequestMapPage() {
  ...
  return (
    <View style={{ flex: 1 }}>
      <NaverMapView ref={mapRef} style={{ flex: 1 }} initialRegion={initialRegion} mapType={mapType} locale='ko' isExtentBoundedInKorea onInitialized={updateScreenPositions} onCameraChanged={updateScreenPositions}>
      ...
      <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
        {helpers.map((row) => {
          ...
          const CARD_WIDTH = 180; const CARD_HEIGHT = 64;
          const jitterX = ((Number(row.id) % 7) - 3) * 10; const jitterY = ((Number(row.id) % 5) - 2) * 8;
          const left = pos.x - CARD_WIDTH / 2 + jitterX; const top = pos.y - CARD_HEIGHT - 12 + jitterY;
          const categories = (row.categories || []).slice(0, 2);
          return (
            <TouchableOpacity key={row.id} activeOpacity={0.9} onPress={() => router.push(`/helper/${row.id}`)} style={[styles.card, { left, top, width: CARD_WIDTH, height: CARD_HEIGHT }]}>
              <View style={styles.cardRow}>
                <Image source={row.users?.thumbnail_url ? { uri: row.users.thumbnail_url } : require('../../../../assets/images/icon.png')} style={styles.avatar} />
                <View style={{ flex: 1 }}>
                  <Typography variant='body' weight='semibold' numberOfLines={1}>{row.name}</Typography>
                  <View style={styles.chipsRow}>
                    {categories.map((id) => {
                      const c = HELP_CATEGORIES.find((x) => x.id === id);
                      const label = c?.displayTitle || c?.label || id;
                      return (<View key={id} style={styles.chip}><Typography variant='caption'>{label}</Typography></View>);
                    })}
                  </View>
                </View>
              </View>
...
const styles = StyleSheet.create({
  card: { position: 'absolute', backgroundColor: '#ffffff', borderColor: '#10B981', borderWidth: 1, borderRadius: 12, padding: 8, shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 3 },
  ...
  chip: { backgroundColor: '#F3F4F6', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2 },
  fab: { position: 'absolute', top: 40, left: 20, backgroundColor: '#333', opacity: 0.48, borderRadius: 24, width: 48, height: 48, justifyContent: 'center', alignItems: 'center', elevation: 5, zIndex: 10 },
});
