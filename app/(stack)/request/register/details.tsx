...
export default function RequestDetailsPage() {
  ...
  const handleNext = () => {
    if (!details.trim()) { Alert.alert('알림', '추가 요청사항을 작성해주세요.'); return; }
    const params = new URLSearchParams({ categories: selectedCategories.join(','), details: details.trim() });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.inputGroup}>
          <TextArea placeholder="예) 가벼운 장보기 동행이 필요합니다. 이동이 불편하여 가까운 마트까지 도움을 받고 싶어요." value={details} onChangeText={setDetails} maxLength={800} style={styles.textArea} />
